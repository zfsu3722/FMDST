...
from Ablation_model.TCN_No_feature_Transformer import TCN_No_feature_Transformer
from Ablation_model.LSTM_No_feature_Transformer import LSTM_No_feature_Transformer

class FMDST_No_feature_Transformer(nn.Module):
    def __init__(self, input_dim=5, output_dim=1, site=134, his=60):
        super(FMDST_No_feature_Transformer, self).__init__()

        # 定义第一个子模型：TCN + 双 Transformer
        self.tcn_model = TCN_No_feature_Transformer(
            input_dim=input_dim,
            output_dim=output_dim
        )

        # 定义第二个子模型：LSTM + 双 Transformer
        self.lstm_model = LSTM_No_feature_Transformer(
            input_dim=input_dim,
            output_dim=output_dim
        )

        # 加载预训练权重（路径根据你的实际情况调整）
        tcn_weight_path = "model_save/Power_Pred_TCN_No_feature_Transformer_his60_Pred1-6_134site_7feature_best_model.pth"
        lstm_weight_path = "model_save/Power_Pred_LSTM_No_feature_Transformer_his60_Pred1-6_134site_7feature_best_model.pth"
