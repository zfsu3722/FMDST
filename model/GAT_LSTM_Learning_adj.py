...
class GAT_LSTM_Learning_adj_Model(nn.Module):
    def __init__(self, input_dim=6, hidden_dim=32, output_dim=1, heads=2, dropout=0.1, num_nodes=10):
        super().__init__()
        self.gat = GATConv(input_dim, hidden_dim, heads=heads, dropout=dropout)
        self.lstm = nn.LSTM(hidden_dim * heads, hidden_dim, batch_first=True)
    ...
    def forward(self, x):
        ...
        x_gat = x_flat.reshape(-1, feat_dim)  # [B*T*N, F]
        ...
        # 5. 空间特征提取
        spatial_features = self.gat(x_gat, batch_edge_index)  # [B*T*N, H*Hd]

        # 6. 时间特征提取
        spatial_features = spatial_features.view(batch_size, seq_len, num_nodes, -1)  # [B, T, N, H*Hd]
        lstm_input = spatial_features.permute(0, 2, 1, 3).reshape(batch_size * num_nodes, seq_len, -1)  # [B*N, T, H*Hd]
